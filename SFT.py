...
def prepare_data_for_sft(instruction_column, response_column, tokenizer, template, max_length=512):
...
def main():
    ...
    output_dir = "./llama3.2-8B-Instruct-sft" # Adjusted output directory
    dataset_path = "sft_dataset" # Path to your SFT dataset on disk.  Assume it's saved as "sft_dataset" using datasets.save_to_disk()
    ...
    local_model_path = model_name # Use model_name directly if you want to download from Hugging Face Hub
    ...
    training_args = TrainingArguments(
        ...
        save_steps=5000,
        save_total_limit=2,
        learning_rate=1e-4,  # Consider adjusting learning rate for SFT, 5e-5 might be too low, 1e-4 or even higher could be tried.
        warmup_steps=1000,
        logging_dir='./logs',
        logging_steps=100,
        gradient_checkpointing=True,  # Enable gradient checkpointing to save memory
        optim="adamw_torch_fused",  # Use fused AdamW for better performance
        fp16=True, # Enable mixed precision training for faster and less memory intensive training. Consider bf16 if your GPUs support it for potentially better stability.
